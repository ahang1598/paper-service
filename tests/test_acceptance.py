...
from tests.conftest import make_chunk, make_fail_body, make_success_body
...
def test_concurrent_queries_do_not_block_event_loop(client, fake_client, monkeypatch):
    ...
    """
    import app.services.document.fulltext_handler as fh
    ...
    async def _run_concurrent(n: int):
        ...
        from app.core.registry import get_handler_class
        from app.core.registry import HandlerContext
        from app.config import Settings

        handler_cls = get_handler_class("doc_fulltext")
        from app.schemas.document import DocFullTextParams
...
def test_log_redaction_masks_secrets(caplog):
    ...
    import logging
    from app.middleware.logging import RedactingFormatter
...
def test_sanitize_mapping_redacts_sensitive_fields():
    ...
    from app.middleware.logging import sanitize_mapping
    data = {
