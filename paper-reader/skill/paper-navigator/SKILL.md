...
固定执行 **3 轮请求**: 第 1 轮网页搜索,论文搜索固定 2 轮(R2/R3);总计最大 3 轮,严禁超过 3 轮。任一轮返回空或无关不得提前终止、不得跳轮或加轮;网页模式仅 R1 执行一次,之后禁止再发网页请求。每轮只调用一次 `WebSearch`,每次 `queries` 中**并行搜索 4 个关键词**。不得少于或多于 4 条;缺口不足时用上位词、同义术语、代表方法、验证词补足。
...
目标: 用网页结果建立术语、同义名、阶段线索。R1 只生成 `R1_context`,不得把网页偶发词直接拼成 R2 主 query。R1 空手或全无关时,不重发网页请求,直接按意图构造 R2,必须继续 R2/R3。
...
R3 是最后一轮论文搜索,仍固定 4 条 query,参数同 R2。R3 根据 R2 审计结果补缺,并承担收口职责:
...
R3 后必须停止,严禁发起第 4 次请求;证据仍缺时在最终回答明示缺口。
...
最终答案只放可读结论,不得暴露或复述检索过程: `R1/R2/R3`、`WebSearch`、`query`、`缺口复盘`、`结果审计` 等过程词,以及"先 R1 探路、再 R2 广度检索"类步骤叙述,一律清除;除非用户明确要求 trace。
...
{"queries":["transformer attention mechanism","self attention sequence transduction","neural machine translation attention","attention based encoder decoder"],"additional_params":{"domain":"paper","search_agent":"creative_qa_agent","agent_size":7,"lang":"en","filter_by":{"pb_time_end":1577836800}}}
```
R3 后必须停止;若"后续主要改进"证据仍不足,最终回答直说不足
