...
from app.api.deps import make_default_device, require_auth
from app.clients.doc_fulltext_client import DocFullTextClient
from app.config import Settings, get_settings
from app.core.exceptions import AppError, UpstreamBusinessError, ValidationError
from app.core.registry import HandlerContext, get_handler_class
...
def _resolve_request_id(req: NavigatorQueryRequest) -> str:
    """透传或自动生成 request_id（srv_ 前缀）。"""
    rid = extract_request_id(req)
    if rid:
        return rid
    return f"{REQUEST_ID_PREFIX}_{int(time.time())}_{uuid.uuid4().hex[:8]}"
...
def get_client_factory(settings: Settings = Depends(get_settings)) -> Callable[[], DocFullTextClient]:
    ...
    """
    from app.api.deps import make_doc_client_factory
